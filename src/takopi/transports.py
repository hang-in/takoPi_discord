...
def _builtin_transports() -> dict[str, TransportBackend]:
    from takopi_discord.backend import BACKEND as discord_backend

    return {discord_backend.id: discord_backend}
...
def get_transport(
    ...
    builtin_transports = _builtin_transports()
    builtin = builtin_transports.get(transport_id)
...
def list_transports(*, allowlist: Iterable[str] | None = None) -> list[str]:
    discovered = list_ids(TRANSPORT_GROUP, allowlist=allowlist)
    return sorted(set(discovered) | set(_builtin_transports()))
